...
# ReDo module
class RedoDense(nn.Module):
    """Dense layer + ReDo running-mean activation tracker."""
    features: int
    name: str
    decay: float = 0.0                # running-mean decay
    ...
    def setup(self):
        ...
        self.variable("redo", f"{self.name}_act", lambda: jnp.zeros((self.features,)))

    def __call__(self, x, *, train: bool):
        y = self.dense(x)
        h = self.act_fn(y)
        if train:
            act = self.get_variable("redo", f"{self.name}_act")
            upd = jnp.mean(jnp.abs(h), axis=0)
            self.put_variable("redo",
                              f"{self.name}_act",
                              act * self.decay + upd * (1.0 - self.decay))
        return h
...
class ActorCritic(nn.Module):
    ...
    def setup(self):
        # --------- ACTOR ----------
        self.a_fc1 = RedoDense(128, decay=self.redo_decay,
                               name="actor_fc1", activation=self.activation)
        self.a_fc2 = RedoDense(128, decay=self.redo_decay,
                               name="actor_fc2", activation=self.activation)
        self.actor_out = nn.Dense(self.action_dim,
                                  kernel_init=orthogonal(0.01),
                                  bias_init=constant(0.0),
                                  name="actor_out")
        # --------- CRITIC ----------
        self.c_fc1 = RedoDense(128, decay=self.redo_decay,
                               name="critic_fc1", activation=self.activation)
        self.c_fc2 = RedoDense(128, decay=self.redo_decay,
                               name="critic_fc2", activation=self.activation)
        self.critic_out = nn.Dense(1,
                                   kernel_init=orthogonal(1.0),
                                   bias_init=constant(0.0),
                                   name="critic_out")

        # Global step counter for the maintenance frequency
        self.variable("redo", "global_step", lambda: jnp.zeros((), jnp.int32))

    def __call__(self, x, *, train: bool):
        # ---------- actor ----------
        h1 = self.a_fc1(x, train=train)
        h2 = self.a_fc2(h1, train=train)
        logits = self.actor_out(h2)
        pi = distrax.Categorical(logits=logits)

        # ---------- critic ----------
        hc1 = self.c_fc1(x, train=train)
        hc2 = self.c_fc2(hc1, train=train)
...
def redo_step(params: FrozenDict,
              redo_state: FrozenDict,
              *,
              rng: jax.random.PRNGKey,
              tau:  float,
              freq: int) -> Tuple[FrozenDict, FrozenDict, jax.random.PRNGKey]:
    """
    One ReDo housekeeping call.
    * keeps a global step counter inside the 'redo' collection
    * every <freq> steps, recycles neurons whose normalised activation ≤ tau
    """
    p, s = unfreeze(params), unfreeze(redo_state)

    def _recycle_step(carry):
        p, s, rng = carry

        def _layer(layer_name, next_layer_name, rng):
            next_root = next_layer_name.split("_d")[0]    # strip '_d' if present
            act = s[layer_name][f"{layer_name}_act"]      # running mean |h|
            total = jnp.sum(act)
            scores = jax.lax.cond(
                total == 0,
                lambda _: jnp.zeros_like(act),
                lambda _: act / total,
                operand=None,
            )
            mask = scores <= tau                          # dormant units
            ...
            if '_out' in next_layer_name:  # handle last layer (actor_out, critic_out) differently
                ...
                W_out = p[next_root][next_layer_name]["kernel"]
            rng, k_init = jax.random.split(rng)
            new_W = weight_reinit(k_init, W_in.shape)

            # apply mask
            W_in  = jnp.where(mask[None, :], new_W, W_in)
            b_in  = jnp.where(mask, 0.0, b_in)
            W_out = jnp.where(mask[:, None], 0.0, W_out)

            # reset tracker
            s[layer_name][f"{layer_name}_act"] = jnp.where(mask, 0.0, act)
            ...
            if "_out" in next_layer_name:
                ...
                p[next_root][next_layer_name]["kernel"] = W_out

            return rng

        # ---- recycle in every hidden layer pair ---------------------------
    ...
    s["global_step"] += 1
    p, s, rng = jax.lax.cond(
        jnp.mod(s["global_step"], freq) == 0,
        _recycle_step,
        _skip_step,
        operand=(p, s, rng),
    )
...
class TrainStateReDo(TrainState):
    """TrainState with an *extra* collection that stores ReDo variables."""
    redo_state: FrozenDict
...
class Config:
    ...
    alg_name: str = "ippo_redo"
    # ------ ReDo (Recycling Dormant neurons) ------
    redo_threshold: float = 0.025  # tau in the paper, dormancy threshold
    redo_freq: int = 10_000          # F in the paper, optimiser steps between sweeps
    redo_decay: float = 0.0         # running‑mean decay (0 for no decay, just batch mean)
...
def main():
        ...
        def run_episode_while(env, key_r, network_params, max_steps=1000):
            ...
            def body_fun(state: EvalState):
                ...
                def select_action(train_state, rng, obs):
                    ...
                    returns the action
                    '''
                    vars_all = {"params": train_state.params, "redo": train_state.redo_state}
    ...
    network = ActorCritic(temp_env.action_space().n, activation=config.activation, redo_decay=config.redo_decay)
    ...
    train_state = TrainStateReDo.create(
        apply_fn=network.apply,
        params=freeze(network_params),
        tx=tx,
        redo_state=freeze(redo_state),
    )

    def apply_net(ts: TrainStateReDo, obs, *, train_flag):
        vars_all = {"params": ts.params, "redo": ts.redo_state}
        (pi, val), mut = ts.apply_fn(vars_all, obs, train=train_flag, mutable=["redo"])
        ts = ts.replace(redo_state=freeze(mut["redo"]))
        return ts, pi, val
            ...
            def _update_epoch(update_state, unused):
                ...
                def _update_minbatch(carry, batch_info):
                    ...
                    def _loss_fn(params, traj_batch, gae, targets):
                        ...
                        vars_all = {"params": params, "redo": train_state.redo_state}
                    ...
                    # ---- ReDo neuron replacement ----
                    rng, redo_rng = jax.random.split(rng)
                    new_p, new_c, _ = redo_step(train_state.params,
                                                train_state.redo_state,
                                                rng=redo_rng,
                                                tau=config.redo_threshold,
                                                freq=config.redo_freq)
                    train_state = train_state.replace(params=new_p, redo_state=new_c)
            ...
            def evaluate_and_log(rng, update_step):
                ...
                def log_metrics(metric, update_step):
                    ...
                    def callback(args):
                        ...
                        ## Tried fixing histogram logging for ReDo layers, doesn't work yet
